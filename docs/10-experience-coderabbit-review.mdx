...
PRを作ると、CodeRabbitが自動でレビューを始めます。この章では、そのレビューを読む → 意味を理解する → 対応する → 再レビューを受けるまでを、実際のサンプルPRを見ながら体験します。
...
まず、PR画面の上のほうに付く大きなコメントです。ここには「このPRが全体として何をしているか」がまとまっています。差分を1行ずつ追う前に、ここで変更の全体像を頭に入れるのが、レビューを読むコツです。
...
> DBに必須の`ogpImageUrl`を持たせ、OGP抽出とローカル保存の処理、画像を保存・配信するAPIエンドポイント、サムネイル表示のクライアント実装とCSS、表示有無のテストを追加した。
...
| 対象ファイル | 内容 |
| --- | --- |
| `src/shared/bookmarks.ts`, `migrations/0003_add_ogp_image.sql`, `src/server/app.ts` | `Bookmark`型に必須の`ogpImageUrl`を追加。マイグレーションで`ogp_image_url`列（NOT NULL、デフォルト`''`）を追加。APIの一覧クエリにも列を反映。 |
| `src/server/ogp.ts`, `src/server/storage.ts`, `test/ogp.test.ts` | `extractOgImageUrl` / `fetchWithTimeout` / `storeOgpImage`を追加。og:image抽出、MIME・サイズ検証、ローカルファイル保存を行い、配信パスか空文字を返す。テストも追加。 |
| `src/server/app.ts`, `test/worker.test.ts` | POST/PUTで`storeOgpImage`を呼び出し、`GET /ogp/:name`で保存済み画像を配信。テストも追加。 |
...
「URL登録 → ページ取得 → og:image抽出 → ローカルに保存 → 一覧表示」という流れが、コードを読まなくても追えます。
...
指摘の本文には、何が問題か・なぜ問題かが書かれています。サンプルPRの指摘をいくつか挙げます。
...
このような提案には、GitHubの画面上に 「Commit suggestion」ボタンが表示されます。押すと、その修正が新しいCommitとしてPRに直接追加されます。手元でコードを書かなくても、ボタン1つで反映できます。

:::warning[ボタンを押す前に、必ず差分を読む]
1クリックで反映できますが、提案が常に正しいとは限りません。何をどう変える提案なのかを自分で読んで理解してから適用してください。理解できない提案をそのまま入れると、後で説明できないコードがPRに残ります。
:::
...
指摘に対応したら、その修正を同じBranchにCommitしてPushします。PRを作り直す必要はありません。
...
Pushすると、CodeRabbitが自動で再レビューを行います。前回の指摘がきちんと直っていれば、その指摘に「✅ Addressed in commit xxxx」と表示され、解決済みとして扱われます。サンプルPRでも、マイグレーションのロールバック注記やテストのクリーンアップの指摘が、後のCommitで対応済みになっています。
...
- Major・セキュリティ（SSRFなど）は優先して対応します。影響が大きいからです。
- Nitpick（ささいな指摘）は取捨選択してかまいません。サンプルPRにも「あるテストケースを足すとなお良い」という程度の指摘があります。
- 対応しないと判断したものは、理由を返信します。コメント欄で `@coderabbitai` に話しかけて質問・相談することもできます。
