...
他サイトの画像URLを直接`<img src>`に指定すると、相手サイトが画像を消したり差し替えたりした瞬間に、こちらの表示も壊れます。これを避けるため、今回は**画像を一度自分の保管場所（Cloudflare R2）にコピーしてから表示**します。R2は[開発環境を準備する](environment-setup)の手順7ですでに作成済みです。
:::
...
| 2 | URLからOGP画像を取得してR2に保存する処理を作る | `src/worker/ogp.ts`（新規） |
| 3 | ブックマーク追加・更新時に2を呼び出し、画像を配信するAPIを足す | `src/worker/index.ts`、`src/shared/bookmarks.ts` |
...
> これは Hono + React + Cloudflare Workers/D1/R2 で動くブックマークアプリです。データベースはD1、画像保存はR2バインディング名 `OGP_BUCKET` を使います。既存のコードのスタイルとコメントの粒度に合わせてください。
:::
...
```prompt
src/worker/ogp.ts を新規作成してください。次の関数をエクスポートします。
...
2. storeOgpImage(pageUrl, bucket, fetcher = fetch): 戻り値は string。
   ...
   - サイズが0、または5MB超なら "" を返す
   - crypto.randomUUID() を使って ogp/<uuid>.<拡張子> というキーで R2 に put する
...
src/worker/index.ts を次のように変更してください。
...
- POST /api/bookmarks と PUT /api/bookmarks/:id で、保存前に
  storeOgpImage(url, c.env.OGP_BUCKET) を呼び、その戻り値を ogp_image_url として保存
- GET /ogp/:name ルートを追加。R2 から ogp/<name> を get し、
...
ダウンロードしたZipを解凍すると、`bookmark-demo-feat-ogp-image`というフォルダができます。これは**完成版のコード一式**です。自分の`bookmark-demo`フォルダとは別の場所に置いておきます。

:::note[GitHub画面からダウンロードする場合]
上のURLが分かりにくければ、[リポジトリのページ](https://github.com/goofmint/bookmark-demo)を開き、左上のBranch切り替えで`feat/ogp-image`を選び、緑色の **「Code」** ボタン →「Download ZIP」でも同じものが取得できます。
:::

### 2. フォルダを差し替える

今回の変更は、すべて次の3つのフォルダの中に収まっています。解凍したフォルダにある、この3つのフォルダで、自分の`bookmark-demo`フォルダの同じフォルダを**まるごと置き換え**ます。
...
具体的には、自分の`bookmark-demo`フォルダにある`src`・`migrations`・`test`をいったん削除し、解凍したフォルダの`src`・`migrations`・`test`をそのままコピーして入れます。

:::warning[置き換えるのはこの3フォルダだけ]
解凍フォルダを**丸ごと**コピーしないでください。`wrangler.toml`の`database_id`など、自分用に設定した値まで上書きされてしまいます（[開発環境を準備する](environment-setup)の手順7で設定した値です）。これらの設定ファイルはルート（`bookmark-demo`直下）にあり、上の3フォルダには含まれないので、3フォルダだけの置き換えなら自分の設定は保たれます。
:::

### 3. 反映して動作確認する

差し替えたら、新しいマイグレーションをデータベースに反映します。
...
表示されたURL（`http://localhost:8787`など）をブラウザで開き、次を確認します。
