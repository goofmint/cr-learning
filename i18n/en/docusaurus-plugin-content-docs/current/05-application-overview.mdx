...
When adding a bookmark, the React screen sends the URL to the API. The API normalizes the URL, fetches the `<title>` of the linked page, and saves it to SQLite. If it finds an OGP image, it stores the image file locally under `data/ogp/`.
