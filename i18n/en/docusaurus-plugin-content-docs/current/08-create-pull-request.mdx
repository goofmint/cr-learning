...
The goal of this chapter is **having a PR created**. Once the PR exists, you can receive a CodeRabbit review in the next chapter.
...
`git status` shows the list of changed and added files, and `git diff` shows the actual diff. Check here that only the files you touched in the previous chapter (`migrations/0003_add_ogp_image.sql`, `src/worker/ogp.ts`, `src/worker/index.ts`, `src/client/App.tsx`, tests, and so on) appear, and that no unintended files are mixed in.
...
Send the Commit to GitHub. Since this is the first time you Push this Branch, add `-u origin branch-name`.
...
- Added processing that fetches the OGP image from a URL and stores it in R2
