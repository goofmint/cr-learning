...
> Added a required `ogpImageUrl` to the DB, processing for OGP extraction and R2 storage, a Worker endpoint to store and serve images, the client implementation and CSS for thumbnail display, and tests for whether it is displayed.
...
| Target file | Content |
| --- | --- |
| `src/shared/bookmarks.ts`, `migrations/0003_add_ogp_image.sql`, `src/worker/index.ts` | Added a required `ogpImageUrl` to the `Bookmark` type. Added an `ogp_image_url` column (NOT NULL, default `''`) via migration. Reflected the column in the Worker's list query. |
| `src/worker/ogp.ts`, `test/ogp.test.ts` | Newly added `extractOgImageUrl` / `fetchWithTimeout` / `storeOgpImage`. Extracts og:image, validates MIME and size, stores to R2, and returns a delivery path or an empty string. Tests added too. |
| `src/worker/index.ts`, `test/worker.test.ts` | Calls `storeOgpImage` on POST/PUT and serves the R2 image via `GET /ogp/:name`. Tests added too. |
...
You can follow the flow of "register URL → fetch page → extract og:image → store to R2 → show in list" without reading the code.
